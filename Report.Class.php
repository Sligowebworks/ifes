...
class Report {
    ...
    function report_chief() {
        $sql = "SELECT
            ...
            elections.round_num,
            country.id AS cid,
            CONCAT('election.php?ID=', elections.id) AS election_link,
            country.region,
            ...
            LCASE(chiefby.the_way) as chief_electedby,
            LCASE(chieftype.the_way) as chief_elected_type,
            ...
            if (sess::get('start_year')!=0 && sess::get('start_month')!=0)
                $sql .= " AND (YEAR(election_date)>='".sess::get('start_year')."' AND MONTH(election_date)>='".sess::get('start_month')."')";
            else if ($start_year!=0)
                $sql .= " AND YEAR(election_date)>='".sess::get('start_year')."'";

            if (sess::get('end_year')!=0 && sess::get('end_month')!=0)
                $sql .= " AND (YEAR(election_date)<='".sess::get('end_year')."' AND MONTH(election_date)<='".sess::get('end_month')."')";
            else if (sess::get('end_year')!=0)
                $sql .= " AND YEAR(election_date)<='".sess::get('end_year')."'";
            ...
            $sqlcurr = "SELECT country.id AS cid,
                country.country_name,
                CONCAT('country.php?ID=', country.id) AS election_link,
                country.region,
                regions.region AS region_name,
                IF(country.show_link=1,
                    CONCAT('<a href=\"country.php?ID=', country.id, '\">', country.country_name, '</a>'),
                ...
                LCASE( chiefby.the_way ) AS chief_electedby, LCASE( chieftype.the_way ) AS chief_elected_type,
                ...
                if (sess::get('byway')>1)
                    $sqlcurr .= " AND country.chief_electedby=".sess::get('byway');

                if (sess::get('etype')>1)
                    $sqlcurr .= " AND country.chief_election_type=".sess::get('etype');

                if (sess::get('term')>0)
                    $sqlcurr .= " AND country.chief_term=".sess::get('term');
            ...
            if($numrows>0) {
                echo '<p>'.$numrows.' dates found.</p>';
            }

            echo '<p style="float:right"><a href="export.php?ID=6" class="tploption">Export Results</a></p>
                <h2>Chief Of State</h2>
                <table class="main-results-table"><thead><tr>
                ...
                foreach($finaldata as $country => $eldata) {
                    if(is_array($currdata)) {
                        if (in_array($country, array_keys($currdata))) {
                            ...
                            echo '<tr><td>';
                            if($data['region_name']!='')
                                echo $data['region_name'].'<br />';
                            echo $data['thecountry'].' </td>
    ...
    function report_govt() {
        ...
        $sql = "SELECT
            ...
            if($numrows>0) {
                ...
                if (sess::get('start_year')!=0 && sess::get('start_month')!=0)
                    $sql1 .= " AND (YEAR(election_date)>='".sess::get('start_year')."' AND MONTH(election_date)>='".sess::get('start_month')."')";
                else if ($start_year!=0)
                    $sql1 .= " AND YEAR(election_date)>='".sess::get('start_year')."'";

                if (sess::get('end_year')!=0 && sess::get('end_month')!=0)
                    $sql1 .= " AND (YEAR(election_date)<='".sess::get('end_year')."' AND MONTH(election_date)<='".sess::get('end_month')."')";
                else if (sess::get('end_year')!=0)
                    $sql1 .= " AND YEAR(election_date)<='".sess::get('end_year')."'";
                ...
                echo '<p>'.$numrows.' dates found.</p>
                <p style="float:right"><a href="export.php?ID=5" class="tploption">Export Results</a></p>
                <h2>Assembly</h2>
                <table class="main-results-table"><thead><tr>
                    <th>Region/ Country</th>
                    <th>Assembly type</th>
                                ...
                                foreach($currefs[$country] as $refid => $refdata) {
                                    foreach($refdata as $house => $housedata) {
                                        ...
                                        if($data1['region_name']!='')
                                            echo $data1['region_name'].'<br />';
                                        echo $data1['thecountry'].' </td>
                                        <td>'.$data1['assembly_t'].'</td>
                                        <td>';
                                        
                                        if ($data1['assembly_t']=='Bicameral') {
                                            echo ($refid=='up'
                                            ? html_entity_decode($data1['uh_name'])
                                            : html_entity_decode($data1['lh_name']));
                                        } else {
                                            echo html_entity_decode($data1['assembly_name']);
                                        }
                                        echo '</td>
                                    ...
                                    foreach($refs[$data['elid']] as $refid => $refdata) {
                                        foreach($refdata as $house => $housedata) {
                                            ...
                                            if ($data['assembly_t']=='Bicameral') {
                                                ...
                                                echo html_entity_decode($data['assembly_name']);
                                            }
                                            echo '</td>
                                            <td>'.$housedata['seats'].'</td>
                                            <td>'.strtolower($housedata['elected_by']).'&nbsp;</td>
    ...
    function report_presidential() {
        ...
        $sql = "SELECT
            ...
            IF ((elections.is_active=4 OR elections.is_active=5),
                CONCAT('Presidential', IF(elections.round_num>0, CONCAT('<br />', round_texts.status_text), ''), ' (',status_texts.status_text, ')'),
                IF(elections.is_active=2,
                    CONCAT('Presidential', IF(elections.round_num>0, CONCAT('<br />', round_texts.status_text), '')),
                    CONCAT('<a href=\"election.php?ID=', elections.id, '\">Presidential</a>',
            ...
            $sql .= " ORDER BY regions.region ASC, country_name ASC, TRIM(candidates_pres.candidate_lname) ASC, elections.order_date DESC";
        //echo $sql;
        $db = new Db();
        $db->Query($sql);
        $numrows = $db->GetAffectedRows();
        ...
        if ($numrows>0) {
            ...
            echo '<p style="float:right"><a href="export.php?ID=3" class="tploption">Export Results</a></p>
            <p>'.$numrows.' candidates Found. </p>
            <table class="main-results-table"><thead><tr>
                <th>Country</th>
                <th>Election Type</th>
                <th>Date</th>
            ...
            while($data = $db->fetchAssoc()) {
                if ($data['round_num']<=11) {
                    ...
                    if ($data['candidate_party']!='')
                        echo html_entity_decode($data['candidate_party']);
                    if ($data['candidate_party']!='' && $data['party_name_foreign']!='')
                        echo '/'.$data['party_name_foreign'];
                    echo '&nbsp;</td>
                    <td>'.html_entity_decode($data['candidate']);
                    echo ($data['is_winner']==1?'<img src="images/active_1.gif" alt="Winner" />':'');
                    echo '</td>
    ...
    function report_parties() {
        ...
        if ($numrows>0) {
            $thetype = $data['election_type'];
            echo '<p style="float:right"><a href="export.php?ID=2" class="tploption">Export Results</a></p>
            <p>'.$numrows.' Parties Found. </p>
            <table class="main-results-table"><thead><tr>
                <th>Country</th>
                <th style="width:60px;">Party Name</th>
                <th>Party Leader</th>
                <th>Election Type</th>
                <th>Date</th>
            ...
            if ($data['round_num']<=11) {
                $reg_voters = ($data['reg_voters']<0) ? 'N/A' : number_format($data['reg_voters'],0);
                    if ($data['assembly_type']=='uni') {
                        ...
                        //    echo $data['region_name'].'/ ';
                        echo $data['thecountry'].' </td>
                        <td>'.(trim($data['party_name'])!=''?html_entity_decode($data['party_name']):'N/A').
                            ($data['is_winner']==1?'<img src="images/active_1.gif" alt="Winner" />':'').'</td>
                        <td>'.(trim($data['party_leader'])!=''?html_entity_decode($data['party_leader']):'N/A').'</td>
                        <td>'.$data['thetype'].'</td>
                        <td>'.$data['thedate'].'</td>
                       	<td>';
                        if ($data['assembly_seats_at_stake']>0) {
                            echo $data['assembly_seats_at_stake'].' seats in '.html_entity_decode($data['assembly_name']);
                        }
                        
                        $diff = $this->get_diff($data['party_seats'],$data['party_seats_won']);
                        echo '<td>'.($data['party_seats_won']=='-1'?'N/A':$data['party_seats_won']).'</td>
                        <!--<td>'.$votes_castp.'</td>-->
                        ...
                        if ($data['show_uh_results']==1) {
                            ...
                            <td>'.(trim($data['party_name'])!=''?html_entity_decode($data['party_name']):'N/A').
                                ($data['is_winner']==1?'<img src="images/active_1.gif" alt="Winner" />':'').'</td>
                            <td>'.(trim($data['party_leader'])!=''?html_entity_decode($data['party_leader']):'N/A').'</td>
                           	<td>'.$data['thetype'].' '.($data['show_lh_results']==1?'<br />'.html_entity_decode($data['uh_name']):'').'</td>
                           	<td>'.$data['thedate'].'</td>
                           	<td>';
                            if ($data['uh_seats_at_stake']>0) {
                                echo $data['uh_seats_at_stake'].' seats in '.html_entity_decode($data['uh_name']);
                            }
                            echo '<td>'.($data['party_seats_won']=='-1'?'N/A':$data['party_seats_won']).'</td>
                            <!--<td>'.$votes_castp.'</td>-->
                            	<td>'.($data['party_seats']=='-1'?'N/A':$data['party_seats']).'</td>
                        ...
                        if ($data['show_lh_results']==1) {
                            ...
                            ($data['is_winner']==1?'<img src="images/active_1.gif" alt="Winner" />':'').'</td>
                            <td>'.(trim($data['party_leader'])!=''?html_entity_decode($data['party_leader']):'N/A').'</td>
                           	<td>'.$data['thetype'].' '.($data['show_uh_results']==1?'<br />'.html_entity_decode($data['lh_name']):'').'</td>
                           	<td>'.$data['thedate'].'</td>
                           	<td>';
                            if ($data['lh_seats_at_stake']>0) {
                                echo $data['lh_seats_at_stake'].' seats in '.html_entity_decode($data['lh_name']);
                            }
                            echo '<td>'.($data['party_seats1_won']=='-1'?'N/A':$data['party_seats1_won']).'</td>
                            <!--<td>'.$votes_castp1.'</td>-->
    ...
    function report_turnout() {
        ...
        if (sess::get('type')=='' || sess::get('type')=='any' || sess::get('type')==4) {
            ...
            WHERE elections.show_results=1
                ...
                AND elections.is_active=1";
            if (sess::get('round_num')>0)
                $sql1 .= " AND elections.round_num=".sess::get('round_num');

            if (sess::get('region')!='')
                $sql1 .= " AND country.region=".sess::get('region');

            if (sess::get('country')!='')
                $sql1 .= " AND country.id=".sess::get('country');
            ...
            if (sess::get('end_year')!=0 && sess::get('end_month')!=0)
                ...
                $sql1 .= " AND YEAR(election_date)<='".sess::get('end_year')."'";
            $sql1 .= " ORDER BY elections.order_date DESC, candidates_ref.order_id ASC";
        ...
        if ($numrows>0) {
            $thetype = $data['election_type'];
            echo '<p style="float:right"><a href="export.php?ID=1" class="tploption">Export Results</a></p>
            <p>'.$numrows.' Elections Found.</p>
            <table class="main-results-table"><thead><tr>
                <th>Region/ Country</th>
                <th>Election Type</th>
                <th>Date</th>
            ...
            if ($data['round_num']<=11) {
                if ($data['election_type']==4) {
                    foreach($refs[$data['id']] as $refid => $refdata) {
                        ...
                        //($refvotes_cast==$refvotes_castp?$refvotes_cast:$refvotes_cast.'<br />'.$refvotes_castp)
                        echo '<tr>
                        <td>';
                        if($data['region_name']!='')
                            echo $data['region_name'].'<br />';
                        echo $data['thecountry'].' </td>
                        <td>'.$data['thetype'].($counter>1?'<br />Provision '.$refdata['order_id']:'').'</td>
                        <td>'.$data['thedate'].'</td>
                    ...
                    if ($data['assembly_type']=='uni') {
                        ...
                        <td>'.$data['thedate'].'</td>
                       	<td>';
                        if ($data['assembly_seats_at_stake']>0) {
                            echo $data['assembly_seats_at_stake'].' seats in '.$data['assembly_name'];
                        }
                        echo '&nbsp;</td>
                       	<td>&nbsp;</td>
                       	<td>'.$votes_castp.'</td>
                        ...
                        if ($data['show_uh_results']==1) {
                            ...
                            echo $data['thecountry'].' </td>
                               	...
                               	<td>';
                            if ($data['uh_seats_at_stake']>0) {
                                echo $data['uh_seats_at_stake'].' seats in '.$data['uh_name'];
                            }
                            echo '&nbsp;</td>
                            	<td>&nbsp;</td>
                            	<td>'.$votes_castp.'</td>
                        ...
                        if ($data['show_lh_results']==1) {
                            ...
                            echo $data['thecountry'].' </td>
                            	...
                            	<td>';
                                if ($data['lh_seats_at_stake']>0) {
                                    echo $data['lh_seats_at_stake'].' seats in '.$data['lh_name'];
                                }
                                echo '&nbsp;</td>
                            	<td>&nbsp;</td>
                            	<td>'.$votes_castp1.'</td>
                    ...
                    echo $data['thecountry'].' </td>
                    	...
                    	<td>'.($data['chief_term']>0?$data['chief_term'].' years':'').' years</td>
                    	<td>'.$votes_castp.'</td>
                    </tr>';
                }
            }
    ...
    function report_average_turnout() {
        ...
        if ($numrows>0) {
            ...
            $thetype = $data['election_type'];
            echo '<!--<p style="float:right"><a href="export.php?ID=7" class="tploption">Export Results</a></p>-->
            <table class="main-results-table"><thead><tr>
                <th>Region</th>
                <th>Election Type</th>
                <th style="width: 100px">Number of Elections</th>
            ...
            <tr>
                ...
                <td>';
                echo ($presctr>0) ? round($prestotal/$presctr,2).'%' : 'N/A';
                echo '</td>
                <td>'.$timerange.'</td>
            </tr>
            <tr>
                ...
                <td>';
                echo ($parlctr>0) ? round($parltotal/$parlctr,2).'%' : 'N/A';
                echo '</td>
                <td>'.$timerange.'</td>
            </tr>
            <tr>
                ...
                <td>';
                echo ($legctr>0) ? round($legtotal/$legctr,2).'%' : 'N/A';
                echo '</td>
                <td>'.$timerange.'</td>
            </tr>
            <tr>
                ...
                <td>';
                echo ($rows['counter']>0) ? round($rows['percentage']/$rows['counter'],2).'%' : 'N/A';
                echo '</td>
                <td>'.$timerange.'</td>
            </tr>
            </tbody></table>';
