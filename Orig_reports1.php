...
<body id="body-<?php echo $section; ?>" onload="<?php echo $onload; ?>">
	...
	<div id="wrapper">
		...
		<div id="main-wrapper">
			...
			<h2>Voter Turnout</h2>
			<p>The first voter turnout report lists voter turnout for specific election dates.
            Select criteria below to narrow your report results.</p>

                <form action="reports1.php" method="get">
            ...
            <br />
            <?php
            if (isset($_GET['submitted']) && (trim($_GET['region'])!='' || trim($_GET['country'])!='')) {
                include('includes/Report.Class.php');
                $rpt = new Report();
                $rpt->report_turnout();
            } else  if (isset($_GET['submitted'])) {
                echo '<p>Region or country is required.</p>';
            }
